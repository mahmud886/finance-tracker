...
export async function getDashboardStats(): Promise<DashboardStats> {
  ...
  for (const tx of rows) {
    ...
    if (!trendMap.has(monthKey)) {
      trendMap.set(monthKey, { month: monthKey, income: 0, expense: 0 });
    }
    ...
    if (tx.type === "income") {
      ...
      if (date >= monthStart) monthlyIncome += amount;
    } else {
      totalExpense += amount;
      trendEntry.expense += amount;
      if (date >= monthStart) monthlyExpense += amount;
  ...
  const [{ data: templates }, { data: previousTemplates }, { data: loans }] = await Promise.all([
    supabase.from("budget_templates").select("id").eq("user_id", user.id).eq("month", monthKey),
    supabase.from("budget_templates").select("id").eq("user_id", user.id).eq("month", previousMonthKey),
    supabase.from("loans").select("id, total_amount, interest_rate, monthly_installment").eq("user_id", user.id).eq("status", "active"),
  ]);
  ...
  if (templateIds.length > 0) {
    ...
    const planned = items.reduce((sum, item) => sum + Number(item.estimated_price), 0);
    const purchasedItems = items.filter((item) => item.is_purchased);
    const spent = purchasedItems.reduce((sum, item) => sum + Number(item.estimated_price), 0);
    const completion = items.length > 0 ? (purchasedItems.length / items.length) * 100 : 0;
  ...
  const previousTemplateIds = (previousTemplates ?? []).map((item) => item.id);
